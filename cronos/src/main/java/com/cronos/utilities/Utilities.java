...
public class Utilities {
	...
	/**
	 * Metodo que valida el digito de verificacion
	 * @autor CACP - RFAST9 14/07/2015
	 * @param nit
	 * @return
	 */
	public static Integer validarDigitoVerificacion(String nit)throws Exception{
		try {
			
			Integer total = 0;
			if(nit != null && !nit.isEmpty()){
				int miArregloPA[] = {3,7,13,17,19,23,29,37,41,43,47,53,59,67,71};
				char[] nitarray = nit.toCharArray();
				int suma = 0; 
				int residuo = 0;		
				
				for(int i = nit.length() -1, j=0; i >= 0; i--, j++){
					suma = suma + Integer.valueOf(String.valueOf(nitarray[i])).intValue() * miArregloPA[j];
				}
				residuo = suma % 11; 
				total = (residuo > 1) ? 11 - residuo : residuo;
			}
			return total;
